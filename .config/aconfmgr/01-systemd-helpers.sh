...
#
# By default, metadata is read from the installed unit. --from-file reads a unit
# installed by this aconfmgr configuration instead.
function SystemdEnable() {
    ...
    # Read either the installed unit or one added by this configuration.
    if [[ $from_file -eq 1 ]]; then
        local unit_source="$output_dir/files/$unit"
    else
        local unit_source="$unit"
    fi
